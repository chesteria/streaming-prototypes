...
describe("LocationFlow State Machine", () => {
  ...
  it("should transition from confirming to complete on confirm_detected", () => {
    const initialState: LocationFlowState = {
      kind: "confirming",
      detectedCity: AUSTIN,
    };
  ...
  it("should exit picker back to confirming when there is a detected city", () => {
    const initialState: LocationFlowState = {
      kind: "picking",
      query: "",
      selectedIndex: 0,
    ...
    const nextState = locationFlowReducer(initialState, {

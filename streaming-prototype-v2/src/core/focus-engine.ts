...
export type KeyAction = 'UP' | 'DOWN' | 'LEFT' | 'RIGHT' | 'OK' | 'BACK' | 'PLAYPAUSE';
...
let currentHandler: ((action: KeyAction, event: KeyboardEvent) => void) | null = null;
let isEnabled = true;

export const enable = () => { isEnabled = true; };
export const disable = () => { isEnabled = false; };

export const setHandler = (handler: (action: KeyAction, event: KeyboardEvent) => void) => {
...
export const createZone = <T>(items: T[], options: FocusZoneOptions<T> = {}): FocusZone<T> => {

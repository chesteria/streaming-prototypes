...
describe("FocusController Adapter", () => {
  ...
  it("should discover and register zones from the DOM", () => {
    ...
    expect(
      document
        .querySelector('[data-focus-id="btn-1"]')
        ?.getAttribute("data-focused"),
    ).toBe("true");
  });

  it("should move real DOM focus onto input elements", () => {
    document.body.innerHTML = `
      <div data-focus-zone="search-zone">
        <input data-focusable="true" data-focus-id="search-input" />
      </div>
    `;

    FocusController.registerZone("search-zone");
    FocusController.focusZone("search-zone");

    expect(document.activeElement).toBe(
      document.querySelector('[data-focus-id="search-input"]'),
    );
  });
  ...
  it("should navigate between items in a zone", () => {
    ...
    expect(
      document
        .querySelector('[data-focus-id="btn-2"]')
        ?.getAttribute("data-focused"),
    ).toBe("true");
  });
  ...
  it("should focus items by ID across zones", () => {
    ...
    expect(
      document
        .querySelector('[data-focus-id="btn-3"]')
        ?.getAttribute("data-focused"),
    ).toBe("true");
  });
  ...
  it("should refresh a zone when DOM changes", () => {
    ...
    expect(
      document
        .querySelector('[data-focus-id="btn-new"]')
        ?.getAttribute("data-focused"),
    ).toBe("true");
  });
});
